...
여러 프로세스가 공유 데이터에 동시에 접근할 때 **Data Inconsistency(데이터 불일치)** 발생한다.

단일 CPU 환경에서 Context Switch로 인해 작업 처리 중간에 다른 프로세스가 끼어드는 경우,  
멀티코어/멀티프로세서 환경에서 분리된 CPU들이 물리적으로 동시에 실행되는 경우가 있다. 

이 상황 모두 Race Condition을 유발할 수 있다.
